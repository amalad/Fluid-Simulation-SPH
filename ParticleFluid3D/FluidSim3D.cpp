...
void Simulation::setDensities()
{
    ...
    for(int i = 0; i < particles.size(); i++)
    {
        ...
        particles[i].density *= 4.0 * m / M_PI / pow(h, 9) ;
    }
}
...
void Simulation::setForces()
{
    ...
    for(int i = 0; i < particles.size(); i++)
    {
        ...
        particles[i].force *= m/(M_PI * pow(h, 5));
    }
}
...
bool square(double x, double y, double z)
{
    return x <= 0.5 && y <= 0.5 && z<=0.5;
}

bool circle(double x, double y, double z)
{
    return ((x - 0.5)*(x - 0.5) + (y - 0.5)*(y - 0.5)) + (z - 0.5)*(z - 0.5) <= 0.25 * 0.25;
}

void Simulation::initPositions()
{
    for(double x = 0.0; x <= 0.3; x += HH)
    {
        for(double y = 0.0; y <= 0.3; y += HH)
        {
            for(double z = 0.0; z <= 0.3; z += HH)
            {
                if(square(x, y, z)){
                    particles.push_back(Particle(dvec3(x, y, z)));
                    cout << x << " " << y << " " << z << endl; 
                }
            }
        }
